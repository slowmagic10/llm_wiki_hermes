# 销售售前 LLM Wiki + Hermes 软件设计文档

## 1. 文档信息

| 项目 | 内容 |
| --- | --- |
| 文档名称 | 销售售前 LLM Wiki + Hermes 软件设计文档 |
| 版本 | v0.1 |
| 日期 | 2026-06-17 |
| 阶段 | 第一版设计 |
| 目标用户 | 销售、售前 |
...
当前目标是构建一套面向销售和售前咨询的内部产品知识问答系统。系统需要基于稳定的产品知识回答问题，并且所有知识类回答必须可追溯到 Obsidian Wiki 中的 Markdown 来源。
...
销售 FAQ 和场景页不复制大段事实，只通过 Obsidian 双链引用事实页：
...
1. 如果 top 结果命中 `Sales/FAQ` 或 `Sales/Scenarios`。
...
阈值需要后续通过测试集校准。第一版可以先使用经验阈值，再根据销售问题测试集调整。
...
## 14. Hermes sales-wiki Profile 设计

### 14.1 Profile 目标

`sales-wiki` profile 只服务销售/售前内部产品知识问答，不影响日常 Hermes 使用。
...
所有产品知识、售前咨询、FAQ 类问题必须调用 Obsidian RAG MCP。
...
2. Hermes 不启用写文件工具。
3. `sales-wiki` 不启用 web/browser/terminal。
4. `sales-wiki` 关闭长期 memory。
5. 知识类回答必须有 RAG 来源。
6. 无来源不回答。
...
2. Hermes `sales-wiki` profile 能发现该工具。
3. Hermes 能调用该工具。
...
当前系统第一版仍然以销售/售前产品知识库为落地场景，但架构目标不是只服务销售场景。后续可以扩展到 HR 制度库、运维手册库、法务知识库、研发知识库等正式知识库场景。
...
第一阶段不迁移当前销售 Vault 目录。未来如果引入第二个领域，Vault 建议演进为：
...
domains:
  sales:
    display_name: 销售售前知识库
    profile: sales
    vault_subpath: domains/sales
    rag_base_url: http://127.0.0.1:18080
    sync_status_file: /root/llm_wiki_hermes/logs/sales-wiki-sync-status.json
    enabled: true
...
shared image:
  sales instance:
    ...
    WIKI_PROFILE=sales
    VECTOR_COLLECTION=sales_chunks
...
1. 不迁移当前销售 Vault 到 `domains/sales`。
2. 不拆当前数据库。
3. 不上线 Milvus。
4. 不新增多领域 RAG 实例。
...
当前阶段只把上述原则写入设计文档和待办，保证当前销售第一版稳定运行。
